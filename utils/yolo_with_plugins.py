...
def _preprocess_yolo(img, input_shape):
    ...
    sho_img = False;
    my_file = Path("./t_origin.txt")
    if not my_file.is_file():
        sho_img=True
        fo = open("t_origin.txt",'w')
        for k in range(channels):
            for i in range(rows):
                for j in range(cols):
                   fo.write(str(img[k,i,j]))
                   fo.write("\n")
        fo.close()
...
#                tempD = detections[i]
#                fo.write(str(tempD[0])+"  " +str(tempD[1]) +"  "+str(tempD[2]) + "  " + str(tempD[3]) +"  "+ str(tempD[4]))
#                fo.write("\n")
#        fo.close()
...
def _postprocess_yolo(trt_outputs, img_w, img_h, conf_th, nms_threshold=0.5):
    ...
    """
#    print("image w is" + str(img_w))
    show_result = False
    my_file = Path("./t_r_3.txt")
    if not my_file.is_file():
         show_result= True
...
#        fo.write("\n")
    ...
    if show_result:
        ...
        for i in range(len(boxes)):
                fo.write("box "+str(boxes[i]))
                fo.write("scores "+str(scores[i]))
                fo.write("classes "+str(classes[i]))
                fo.write("\n")
        fo.close()
