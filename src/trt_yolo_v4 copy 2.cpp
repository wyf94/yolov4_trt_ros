...
private:
    ...
    void init_params()
    {
        ...
        image_sub_0 = new ImageSubscriber(n_, video_topic_0, 1);
        image_sub_1 = new ImageSubscriber(n_, video_topic_1, 1);
...
void Yolov4::imageSyncCallback(const sensor_msgs::ImageConstPtr &msg_0, const sensor_msgs::ImageConstPtr &msg_1)
{
    ...
    cv::Point p_start = cv::Point(10, 40);
    std::string str_start = std::to_string(start_time);
    cv::putText(img_0, str_start, p_start, cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 255), 2);
    cv::Point p_end = cv::Point(10, 70);
    std::string str_end = std::to_string(end_time);
    // std::cout<<"start_time: "<<str_start<<"  end_time: "<<str_end<<std::endl;
    cv::putText(img_0, str_end, p_end, cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 255), 2);
